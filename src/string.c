...
int
strcmpLimits(char* data, char* comparison, int from, int to) {
	int i, compI;
	int auxLen = strlen(comparison)+1;
	char aux[auxLen];

	aux[auxLen] = '\0';
	for (i = from, compI = 0; data[i] == comparison[compI] && i < to; i++, compI++) {
		if (data[i] == '\0')
		return 0;
	}
	return data[i] - comparison[compI];
}
